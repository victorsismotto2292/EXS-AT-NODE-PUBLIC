// Tentativa 1 - Alterar Dados no JSON:
...
app.post('/carros-luxo/atualizar-carro', (req, res) => {
    ...
    if(luxosIndex === -1){
        res.send(`<h1>Carro de luxo não encontrado, por favor, tente novamente.</h1>`);
        return;
    }
    else{
...
app.post('/carros-esportivo/atualizar-carro', (req, res) => {
    ...
    let esportivoData = fs.readFileSync(esportivosPath, 'utf-8');
    let esportivos = JSON.parse(esportivoData);

    const esportivosIndex = esportivos.findIndex(carro => carro.nome.toLowerCase() === nome.toLowerCase());

    if(esportivosIndex === -1){
        res.send('<h1>Carro esportivo não encontrado, por favor, tente novamente.</h1>');
        return;
    }
    else{
...
app.post('/carros-classico/atualizar-carro', (req, res) => {
    ...
    if(classicosIndex === -1){
        ...
        res.send(`<h1>Dados do carro clássico atualizados com sucesso!</h1>`);
    }
});
